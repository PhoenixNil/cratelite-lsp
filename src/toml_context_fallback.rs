...
pub fn get_version_context(text: &str, line: u32, character: u32) -> Option<VersionContext> {
    ...
    if matches!(key, "version" | "path" | "git" | "branch" | "tag" | "rev" | "edition") {
        return None;
    }
    ...
    Some(VersionContext {
        ...
        range: Range::new(position_of(&ls, abs_content_start), position_of(&ls, abs_content_end)),
    })
}
...
fn parse_bare_key<'a>(text: &'a [u8], offset: usize, end: usize) -> Option<(&'a str, usize)> {
    let start = offset;
...
fn parse_toml_str(text: &[u8], offset: usize, end: usize) -> Option<ParsedStr> {
    ...
    while cursor < end {
        ...
        if b == delim && !escaped {
            return Some(ParsedStr { value, content_start, content_end: cursor, next_offset: cursor + 1 });
        }
    ...
    Some(ParsedStr { value, content_start, content_end: end, next_offset: end })
}
...
fn find_matching(text: &[u8], open_at: usize, open: u8, close: u8, end: usize) -> Option<usize> {
    ...
    while cursor < end {
        ...
        if in_str {
            ...
            if b == delim && !escaped { in_str = false; }
            escaped = false;
            cursor += 1;
            continue;
        }
        ...
        if b == b'#' { in_comment = true; cursor += 1; continue; }
        if b == open  { depth += 1; cursor += 1; continue; }
        if b == close {
            if depth == 0 { return None; } // shouldn't happen
            depth -= 1;
...
fn skip_toml_value(text: &[u8], offset: usize, end: usize) -> usize {
    ...
    let b = text[offset];
    if is_str_delim(b) {
        return parse_toml_str(text, offset, end).map(|s| s.next_offset).unwrap_or(end);
    }
    if b == b'[' {
        return find_matching(text, offset, b'[', b']', end).map(|i| i + 1).unwrap_or(end);
    }
    if b == b'{' {
        return find_matching(text, offset, b'{', b'}', end).map(|i| i + 1).unwrap_or(end);
    }
...
fn parse_feature_array(
    ...
    if offset >= end || text[offset] != b'[' { return None; }
    ...
    while cursor < close {
        ...
        if !is_str_delim(text[cursor]) { cursor += 1; continue; }
...
fn parse_inline_table(text: &[u8], open: usize, close: usize, cursor_offset: usize) -> InlineTable {
    let mut result = InlineTable { package_name: None, version: None, feature_array: None };
    ...
    while cursor < close {
        ...
        if (key == "package" || key == "version") && val_start < close && is_str_delim(text[val_start]) {
            if let Some(s) = parse_toml_str(text, val_start, close) {
                if key == "package" { result.package_name = Some(s.value.clone()); }
                else { result.version = Some(s.value.clone()); }
        ...
        if key == "features" && val_start < close && text[val_start] == b'[' {
            ...
            cursor = result.feature_array.as_ref().map(|f| f.next_offset)
...
fn find_inline_dep_start(text: &str, line: u32, character: u32) -> Option<InlineDepStart> {
    ...
    for l in (0..=(line as usize)).rev() {
        ...
        if brace_offset >= bytes.len() { continue; }
...
pub fn get_feature_completion_context(text: &str, line: u32, character: u32) -> Option<FeatureCompletionContext> {
